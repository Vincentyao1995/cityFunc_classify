...
def split_cityFunc(image_num = 99999):
    image_root_path = '../dataset/cityFuncDataset/train/'
    image_new_path = '../dataset/cityFuncDataset/new_train/'

    if not os.path.exists(image_new_path):
        os.mkdir(image_new_path)

    # total_img_num = 0
    # for image_class in os.listdir(image_root_path):
    #     imgClass_path = os.path.join(image_root_path, image_class)
    #     total_img_num += len(os.listdir(imgClass_path))
    #
    # each_img_sub_num = int(image_num/total_img_num)

    for image_class in os.listdir(image_root_path):
        imgClass_path = os.path.join(image_root_path,image_class)
        imgClass_new_path = os.path.join(image_new_path, image_class)
        if not os.path.exists(imgClass_new_path):
            os.mkdir(imgClass_new_path)
        for img in os.listdir(imgClass_path):
            #process this img, crop it randomly.
            img_path = os.path.join(imgClass_path, img)
            random_cut_img(img_path, imgClass_new_path)



def random_cut_img(image_old_path, image_new_path, target_img_size = 256):

    print('processing %s' %(image_old_path))

    g_count = len(os.listdir(image_new_path))
    count = 0
    image = cv2.imread(image_old_path)

    X_height, X_width = image.shape[0], image.shape[1]
    num_generate_img = int(max(X_height, X_width)/target_img_size*10)

    while count < num_generate_img:
        if X_height <= target_img_size:
            if X_width > target_img_size:
        ...
        if X_width <= target_img_size:
            if X_height > target_img_size:
                ...
                random_width = 0
        if min(X_width, X_height) > target_img_size:
            random_width = random.randint(0, X_width - size - 1)
            random_height = random.randint(0, X_height - size - 1)
        image_ogi = image[random_height: random_height + size, random_width: random_width + size,:]

        image_d, _ = data_augment(image_ogi, image_ogi)

        cv2.imwrite((image_new_path + os.sep + '%05d.png' % g_count), image_d)
...
if __name__ == '__main__':
    ...
    # generate_train_dataset(image_num=99999)
